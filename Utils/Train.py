...
from Utils.Models import CNN1D, CNN2D, LSTM
...
def train(X,y, sample_rate, delta_time, batch_size,model_type):
    params = {'N_CLASSES':88,
             'SR':sample_rate,
             'DT':delta_time}

    models = {'CNN1D':CNN1D(**params),
              'CNN2D':CNN2D(**params),
              'LSTM':  LSTM(**params)}
    ...
    csv_path = os.path.join('logs', '{}_log.csv'.format(model_type))
    csv_logger = CSVLogger(csv_path, append=False)
    model.fit(tg, validation_data=vg,epochs=30, verbose=1,callbacks=[csv_logger, cp])

if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='Musical audio converter')
    parser.add_argument('--model_type', type=str, default='lstm', help='model to run. [CNN1D,CNN2D,LSTM]')
    parser.add_argument('--src_root', type=str, default='clean', help='directory of audio files')
